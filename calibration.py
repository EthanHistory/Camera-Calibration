...
def init_camera_matrix(objpoints:List[np.array], imgpoints:List[np.array], skewless=True) -> np.array :
...
def init_camera_extrinsics(objpoints:List[np.array], imgpoints:List[np.array], skewless=True) -> np.array :
    Hs = n_view_homographies(objpoints, imgpoints)
    K = init_camera_matrix(objpoints, imgpoints, skewless)
